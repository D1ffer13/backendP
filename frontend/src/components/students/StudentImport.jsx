...
import { downloadStudentTemplate } from '../../utils/excelParser';
...
const StudentImport = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      setLoading(true);
      const response = await studentService.importFromExcel(file);
      setResult(response);
      
      if (response.errors === 0) {
        alert(`Успешно импортировано ${response.imported} учеников`);
        onSuccess();
      } else {
        alert(`Импортировано ${response.imported} учеников. Ошибок: ${response.errors}`);
      }
    } catch (error) {
      console.error('Error importing students:', error);
      alert('Ошибка при импорте учеников');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Импорт учеников из Excel">
      <div style={{ padding: '20px 0' }}>
        <div style={{ marginBottom: '20px' }}>
          ...
          <p style={{ marginTop: '10px', fontSize: '14px', color: '#666' }}>
            Скачайте шаблон, заполните данные учеников и загрузите обратно
          </p>
        </div>
